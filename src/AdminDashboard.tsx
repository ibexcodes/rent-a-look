...
export default function AdminDashboard({ onBack }: AdminDashboardProps) {
  ...
  const handleUpdateTransactionStatus = async (transaction: any, status: string) => {
    try {
      ...
      if (status === 'cancelled' && transaction.itemId) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          <div>
            ...
            <p className="text-black/60">Real-time overview of RENT-A-LOOK</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            {!isSpecificAdmin && (
              <button
                ...
                className="px-4 py-2 bg-black text-white rounded-xl text-sm font-bold hover:bg-botswana-blue transition-colors"
              >
                Seed Database
              </button>
            )}
                  ...
                  <tr key={t.id} className="hover:bg-black/[0.01] transition-colors">
                    ...
                    <td className="px-8 py-4">
                      <div className="flex gap-2">
                        ...
                        {t.status === 'completed' && t.type === 'rental' && (
                          <button 
                            onClick={() => handleUpdateTransactionStatus(t, 'returned')}
              ...
              <tbody className="divide-y divide-black/5">
                {inventory.map((item) => (
                  <tr key={item.id} className="hover:bg-black/[0.01] transition-colors">
                    <td className="px-8 py-4">
                      <div className="flex items-center gap-4">
                        <img src={item.image || item.imageUrl || `https://picsum.photos/seed/${encodeURIComponent(item.name)}/400/500`} alt="" className="w-10 h-10 rounded-lg object-cover" />
        ...
        {deleteConfirmId && (
          <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
            ...
            >
              ...
              <h3 className="text-2xl font-display font-bold mb-2">Delete Item?</h3>
              <p className="text-black/60 mb-8 text-sm">This action cannot be undone. Are you sure you want to remove this item from inventory?</p>
              <div className="flex gap-4">
                <button 
                  onClick={() => setDeleteConfirmId(null)}
                  className="flex-1 py-3 bg-black/5 rounded-xl font-bold hover:bg-botswana-blue/10 transition-colors"
                >
                  Cancel
                </button>
                <button 
